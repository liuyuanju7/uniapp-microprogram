<template>
	<view>
		<u-popup :show="isShow" mode="bottom" :round="10" @close="closeLogin" zIndex="999998">
			<view class="f__login" style="padding: 30rpx;">
				...
				<view class="text">登录后享受更好的服务体验</view>
				<view class="loginButton" v-if="!isPhoneLogin">
					...
					<button class="button marginT" @click="onAuthorization"
						:style="{background:PrimaryColor}">微信授权登录</button>
	...
	export default {
		...
		methods: {
			...
			onAuthorization(e) {
				...
				uni.login({
					...
					success: function(loginRes) {
						...
						if (loginRes.errMsg == "login:ok") {
							that.common.http('wxlogin/login', {
								code: loginRes.code
							}, function(res) {
								console.log(res);
								if (res.code !== 200) {
				...
				return;
				this.common.wxLogin(function(res) {
					console.log(res);
					that.common.value('user', res.data);
				});
				// wx.getUserProfile({
